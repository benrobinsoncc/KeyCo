...
class KeychainHelper {
    ...
    static func storeAPIKey(_ key: String) -> Bool {
        ...
        // This is more reliable for Keychain Sharing between app and extension
        // The entitlements file will handle the access group matching
        
        NSLog("[KeychainHelper] [App] Storing API key (using access group from entitlements)")
        NSLog("[KeychainHelper] [App] Bundle ID: \(Bundle.main.bundleIdentifier ?? "nil")")
        NSLog("[KeychainHelper] [App] Service: \(service), Account: \(apiKeyKey)")
        let status = SecItemAdd(query as CFDictionary, nil)
        
        if status == errSecSuccess {
            NSLog("[KeychainHelper] ✅ API key stored successfully with access group: \(accessGroup)")
            return true
        } else {
            NSLog("[KeychainHelper] ❌ Failed to store API key - Status: \(status), Access group: \(accessGroup)")
            return false
        }
    }
    ...
    static func retrieveAPIKey() -> String? {
        ...
        // DON'T specify access group - let iOS use the one from entitlements automatically
        // This matches how we store it (without explicit access group)
        
        NSLog("[KeychainHelper] [App] Retrieving API key (using access group from entitlements)")
